...
class DMATester(dut: DMA) extends PeekPokeTester(dut) {
  ...
  val memHeight     = Array(5, 16, 10)
  val memWidth      = Array(12, 4, 10)
  ...
  for (i <- 0 until numOfConfigs)
    if ((memHeight(i)*memWidth(i)) % localMemSize != (memHeight(i)*memWidth(i)))
  ...
  for (config <- 0 until numOfConfigs) {
    ...
    if (logConfig) {
      ...
      println("[DMATester]   matrix height    : " + memHeight(config))
      println("[DMATester]   matrix width     : " + memWidth(config))
    ...
    poke(dut.io.ctrl.ldHeight, memHeight(config).U)
    poke(dut.io.ctrl.ldWidth, memWidth(config).U)
    ...
    expect(dut.io.bus.busBurstLen, (memHeight(config)*memWidth(config)/dmaChannels).U)
    // - Local memory interface:
    expect(dut.io.memSel, targetMemIsA(config).B)
    for (ch <- 0 until dut.getChannels) {
      expect(dut.io.addr(ch), (ch % localMemSize).U)
    ...
    breakable {
      if (memHeight(config)*memWidth(config) == 0) {
        ...
        var rowCntr = 0
        var columnCntr = 0
        for (cycle <- 0 until memHeight(config)*memWidth(config)/dmaChannels) {
          ...
          if (cycle != 0 && ((columnCntr+dmaChannels) < memWidth(config))) {
            columnCntr += dmaChannels
          } else if (cycle != 0) {
            columnCntr = 0
            rowCntr += 1
          }
          for (ch <- 0 until dut.getChannels) {
            expect(dut.io.addr(ch), ((rowCntr*gridSize + columnCntr + ch) % localMemSize).U)
  ...
  for (config <- 0 until numOfConfigs) {
    ...
    breakable {
      if ((memHeight(config)*memWidth(config)) == 0) {
        ...
        var rowCntr = 0
        var columnCntr = 0
        for (cycle <- 0 until (memHeight(config)*memWidth(config))/dmaChannels-1) {
